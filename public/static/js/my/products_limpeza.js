var xhrLimpeza = new XMLHttpRequest();
xhrLimpeza.open('GET', `http://localhost:5005/products_varied?qtd=3`, true); // false for synchronous request
xhrLimpeza.setRequestHeader('Content-Type', 'application/json');
xhrLimpeza.send(null);

xhrLimpeza.onreadystatechange = function () {
    var response = xhrLimpeza.responseText;
    try {
    ...
    if (xhrLimpeza.readyState === XMLHttpRequest.DONE && xhrLimpeza.status === 200) {
        for (var i = 0; i < 3; i++) {
            createProductLimpeza(i, obj[i]);
            console.log(obj[i]);
...
function createProductLimpeza(count, obj) {
    ...
    div1.className = `col-xl-3 col-lg-4 col-md-4 col-12 limpeza-${count}`;
    ...
    document.querySelector(`.products-limpeza`).appendChild(div1);
}