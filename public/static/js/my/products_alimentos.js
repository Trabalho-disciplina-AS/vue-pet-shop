var xhrAlimentos = new XMLHttpRequest();
xhrAlimentos.open('GET', `http://localhost:5005/products_varied?qtd=3`, true); // false for synchronous request
xhrAlimentos.setRequestHeader('Content-Type', 'application/json');
xhrAlimentos.send(null);

xhrAlimentos.onreadystatechange = function () {
    var response = xhrAlimentos.responseText;
    try {
    ...
    if (xhrAlimentos.readyState === XMLHttpRequest.DONE && xhrAlimentos.status === 200) {
        for (var i = 0; i < 3; i++) {
            createProductalimentos(i, obj[i]);
            console.log(obj[i]);
...
function createProductalimentos(count, obj) {
    ...
    div1.className = `col-xl-3 col-lg-4 col-md-4 col-12 alimentos-${count}`;
    ...
    document.querySelector(`.products-alimentos`).appendChild(div1);
}