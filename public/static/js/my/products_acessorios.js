var xhrAcessorios = new XMLHttpRequest();
xhrAcessorios.open('GET', `http://localhost:5005/products_varied?qtd=3`, true); // false for synchronous request
xhrAcessorios.setRequestHeader('Content-Type', 'application/json');
xhrAcessorios.send(null);

xhrAcessorios.onreadystatechange = function () {
    var response = xhrAcessorios.responseText;
    try {
    ...
    if (xhrAcessorios.readyState === XMLHttpRequest.DONE && xhrAcessorios.status === 200) {
        for (var i = 0; i < 3; i++) {
            createProductAcessorios(i, obj[i]);
            console.log(obj[i]);
...
function createProductAcessorios(count, obj) {
    ...
    div1.className = `col-xl-3 col-lg-4 col-md-4 col-12 acessorios-${count}`;
    ...
    var view = document.createTextNode("Compra Rápida");
    ...
    var modal = document.querySelector(`.products-acessorios`)
    console.log("MODAL AQUI DENIS: " + modal);
    console.log("TIPO AQUI DENIS: " + typeof modal);
    document.querySelector(`.products-acessorios`).appendChild(div1);
}

// function createModelAcessorio(name) {
//     var document.querySelector(`.modal-content`)
// }